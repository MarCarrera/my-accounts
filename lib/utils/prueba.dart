...
///CODIGO DE PRUEBA EN DIALOGO DE PÁGOS ----------------------------------
/*
Para importar y utilizar la clase `Prueba` (que contiene el `BoardDateTimeBuilder`) en otro widget, y hacer que aparezca en el `customView` del diálogo `Dialogs.materialDialog`, debes crear una instancia del widget que deseas mostrar dentro del diálogo.

Aquí hay un ejemplo de cómo hacerlo:

1. **Crea el Widget de la clase `Prueba`**:
   Primero, asegúrate de que la clase `Prueba` y sus dependencias estén correctamente configuradas. Aquí está la clase `Prueba` que ya proporcionaste:
...
2. **Utiliza `Prueba` en Otro Widget**:
   Ahora, para

 abrir el diálogo y mostrar el contenido de `Prueba` en el `customView`, necesitas asegurarte de que `Prueba` esté instanciado correctamente. Puedes hacerlo de la siguiente manera:
   ...
   En este ejemplo, el botón `Seleccionar Fecha` dentro del diálogo abre un `BottomSheet` que contiene el widget `Prueba`, permitiendo así seleccionar la fecha desde el picker que has implementado en la clase `Prueba`.

Con este enfoque, podrás abrir el diálogo y mostrar el picker de fecha dentro de un `BottomSheet` al hacer clic en el botón `Seleccionar Fecha`. Si deseas una integración más directa dentro del propio diálogo, tendrás que modificar el contenido de `Prueba` para adaptarlo al `customView` del diálogo.
 */